...
#실습 2-19
select * from `bank_transaction` where `t_dist` <> 3 order by `t_dist` asc, `t_amount` desc;
#실습 2-20
select 
	count(case when `t_dist` = 1 then 1 end) as `입금 건수`, # count(if(`t_dist`=1, 1, null)) as `입금 건수` 도 가능
